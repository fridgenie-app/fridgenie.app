...
the new primary domain **myjujube.app** (registered via GoDaddy).

## What was NOT changed automatically (and why)

- **`CNAME` still contains `fridgenie.app`.** It was left as-is on purpose:
  GitHub Pages binds one custom domain per repo, and `myjujube.app` DNS is not
  pointed at GitHub Pages yet. Flipping `CNAME` before `myjujube.app` resolves
...
1. **Point `myjujube.app` DNS at GitHub Pages** (A records
   `185.199.108–111.153`, plus `www` CNAME → `<user>.github.io`), or move to
   Cloudflare Pages / Netlify. The domain is already registered at GoDaddy.
2. **Point this repo at the new domain:** set repo **Settings → Pages → Custom
   domain** to `myjujube.app`, and update the `CNAME` file to `myjujube.app`.
   Enable "Enforce HTTPS".
3. **Redirect the old domain `fridgenie.app` → `myjujube.app` (301).**
   GitHub Pages cannot emit a true 301, so pick one:
   - **Registrar/Cloudflare URL forwarding (recommended, real 301):** add a
     301 forward rule `fridgenie.app/* → https://myjujube.app/$1` at the DNS
   ...
   - **Static fallback (last resort):** deploy a tiny site on `fridgenie.app`
     ...
     `<link rel="canonical" href="https://myjujube.app/...">`.
4. **Verify:** `curl -sI https://fridgenie.app/` returns `301` → `myjujube.app`,
   and `https://myjujube.app` serves the Jujube site over HTTPS.

## Supabase (must be updated in the dashboard before shipping the app)

The mobile app now sends users to `https://myjujube.app/auth/confirmed`. For
email confirmation / OAuth to work:
- **Auth → URL Configuration → Site URL** → `https://myjujube.app`
- **Auth → URL Configuration → Redirect URLs** → add
  `https://myjujube.app/**` and `com.fridgenie.app://callback` (keep the old
  `https://fridgenie.app/**` until the 301 is live).
- **Auth → Email Templates** → update any hardcoded `fridgenie.app` links and
  the "Fridgenie" wording to "Jujube" / `myjujube.app`.

## Contact email

Pages now show `hello@myjujube.app`. Set up a mailbox or forwarder (MX records)
for `myjujube.app` so this address does not bounce — it appears in the privacy
policy and terms.
